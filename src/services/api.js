...
// Funções de Autenticação de Teste/Validação (se ainda forem necessárias)
export const testAuth = async () => {
  try {
    console.log('[API] testAuth chamado.');
    const testResponse = await api.get(`/auth/test`); // Assegure que este endpoint exista no backend
    return { status: 'ok', authenticated: true, data: testResponse.data };
  } catch (e) {
    console.error('[API] Erro em testAuth:', e.response?.data || e.message);
    return { status: 'error', authenticated: false, error: e.response?.data || e.message };
  }
};

export const validateToken = async () => {
  console.log('[API] validateToken chamado.');
  try {
    const token = await getAuthToken();
    if (!token) {
      return { valid: false, error: 'Token não disponível no frontend' };
    }
    // Esta chamada é feita com axios diretamente, sem o interceptor da instância 'api',
    // pois o token é enviado no corpo.
    const response = await axios.post(`${API_URL}/auth/validate-token`, { token }, { // Assegure que este endpoint exista
      headers: { 'Content-Type': 'application/json' }
    });
    return response.data;
  } catch (error) {
    console.error('[API] Erro em validateToken:', error.response?.data || error.message);
    return { 
      valid: false, 
...
export const checkAuth = async () => { // Similar a testAuth
  console.log('[API] checkAuth chamado.');
  try {
    const response = await api.get('/auth/test'); // Assegure que este endpoint exista
    return response.data;
  } catch (error) {
    console.error('[API] Erro em checkAuth:', error.response?.data || error.message);
